...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset ($_POST['form_type']) && $_POST['form_type'] === 'workshop') {
    $author = $_POST["author"];
    ...
    if (isset ($id) && !empty ($id)) {
        $query = "UPDATE workshop SET author=?, title=?, conf_name=?, vol_pp=? WHERE Id=? AND entity=?";
    } else {
        $query = "INSERT INTO workshop (author, title, conf_name, vol_pp, entity) VALUES (?, ?, ?,?, ?)";
    }
    ...
    if (!$stmt) {
        ...
        if (isset ($id) && !empty ($id)) {
            mysqli_stmt_bind_param($stmt, 'ssssis', $author, $title, $conf_name,  $vol, $id, $userRole );
        } else {
...
<head>
    <script>
        $(document).ready(function () {
            $("#workshop2").submit(function (event) {
            ...
            function validateForm() {
                var author = document.forms["workshop2"]["author"].value;
                var conf_name = document.forms["workshop2"]["conf_name"].value;
                var ic = document.forms["workshop2"]["title"].value;
                var nc = document.forms["workshop2"]["vol_pp"].value;
...
<body>
    <div class="d-flex flex-column align-items-center w-100">
        <p class="form-name">Workshops Publications for the year 2023-24</p>
        <div class="form-container">
            <form style="" id="workshop2" action="" method="post" onsubmit="return validateForm();">
            <input type="hidden" name="Id" value="<?php echo isset ($_GET['dataId']) ? $_GET['dataId'] : null; ?>">
                <input type="hidden" name="form_type" value="workshop">
                ...
                <div class="row mb-3">
                    <label for="conf_name" class="col-sm-2 col-form-label label-class">Name of Workshop:
                    </label>
                    <div class="col-sm-10">
                        <input type="text" name="conf_name" placeholder="Name of Workshop"
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-sm-10">
                        <input type="text" name="vol_pp" placeholder="vol_pp, Year, P.P." class="form-control"></input>
                    </div>
                </div>
